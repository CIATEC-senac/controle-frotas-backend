...
export class UserService {
  ...
  async seed() {
    ...
    const constroiAleatorio = (dados: string, min: number, max: number) => {
      //aqui vai ser onde vamos gerar o dado fake em 'dados:string'
      ...
      for (let i = 0; i < tamanho; i++) {
        // cria um loop com base no numeto da const tamanho
    ...
    for (let i = 0; i <= 100; i++) {
      await this.create(constroiUsuarioAleatorio()).catch(
        (
          // eslint-disable-next-line @typescript-eslint/no-unused-vars
          e, //aqui cria o usuario fake, atravez do await ele espera a opração esta finalizada para começar a proxima
